...
public class Solution {
  public record Card(int id, List<Integer> winning, List<Integer> have, int points, int copies) {
    ...
    static Card fromLine(String line) {
      ...
      List<Integer> winning = Arrays.stream(sets[0].split("\\s+")).filter(e -> !e.isEmpty()).map(Integer::parseInt).toList();
      List<Integer> have = Arrays.stream(sets[1].split("\\s+")).filter(e -> !e.isEmpty()).map(Integer::parseInt).toList();
    ...
    String asString() {
      return STR. "Card \{ id }: \{ Arrays.toString(winning.toArray()) } | \{ Arrays.toString(have.toArray()) }" ;
    }
  }
  ...
  /**
   * Part two sucked because I forgot that points in part one weren't just the number of matches, but a power of 2. So I was
   * adding (incorrectly) the original points from part one, instead of just the number of matches found for a given card.
   */
  public static BigInteger partTwo(List<Card> cards) {
    ...
    for (int i = 0; i < cards.size() - 1; i++) {
      ...
      int finalI = i;
      IntStream.range(i + 1, until).forEach(copy -> cardCounts[copy] = cardCounts[copy].add(cardCounts[finalI]));
    }
