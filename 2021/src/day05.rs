...
impl Line {
    ...
    fn get_points_iter(&self) -> Box<dyn Iterator<Item = (i32, i32)> + '_> {
        ...
        if x1 == x2 {
            Box::new((y1.min(y2)..=y1.max(y2)).map(move |y| (x1, y)))
        } else if y1 == y2 {
            Box::new((x1.min(x2)..=x1.max(x2)).map(move |x| (x, y1)))
        } else {
            Box::new((0..=(x1 - x2).abs()).map(move |d| {
                (
...
pub fn solve(lines: Vec<String>) -> (usize, usize) {
    ...
    let (vert_or_horiz, diag): (Vec<&Line>, Vec<&Line>) =
        vents.iter().partition(|l| l.x1 == l.x2 || l.y1 == l.y2);
...
fn add_overlaps(vents: &[&Line], overlaps: &mut Box<[[u8; 1000]; 1000]>) {
    for vent in vents {
